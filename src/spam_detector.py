...
import os
def obtener_ruta_datos(): # Obtiene la ruta al archivo de datos
    script_dir = os.path.dirname(__file__) #src/
    proyect_dir = os.path.dirname(script_dir) #spam_detector/
    return os.path.join(proyect_dir, 'data', 'spam.csv')
def entrenar_modelo():
...
def predecir_spam(texto, modelo, vectorizer):
    ## Predice si el texto es un spam o no 
